    ...
    SubShader
    {
       ...
       Pass
       {
           ...
           v2f vert(appdata_t v)
           {
               ...
               Out.texcoord = TRANSFORM_TEX(v.vertex.xy, _MainTex);
           ...
           fixed4 sobel(v2f v)
           {
               //UVç¿ïWÇ≈ÇÃ1pxÇÕ_MainTex_TexelSize.xy uv atlasÇ≈Ç†Ç¡ÇƒÇ‡Ç±ÇÃíPà ÇÕïœÇÌÇÁÇ»Ç¢
               float dx = _MainTex_TexelSize.x * _scaleFactor.x;
               float dy = _MainTex_TexelSize.y * _scaleFactor.y;

               half4 c00rgba = tex2D(_MainTex, v.texcoord + half2(-dx, -dy));
               half c00 = luminance(c00rgba);

               half4 c01rgba = tex2D(_MainTex, v.texcoord + half2(-dx, 0.0));
               half c01 = luminance(c01rgba);

               half4 c02rgba = tex2D(_MainTex, v.texcoord + half2(-dx , dy));
               half c02 = luminance(c02rgba);

               half4 c10rgba = tex2D(_MainTex, v.texcoord + half2(0.0, -dy));
               half c10 = luminance(c10rgba);

               half4 c12rgba = tex2D(_MainTex, v.texcoord + half2(0.0, dy));
               half c12 = luminance(c12rgba);

               half4 c20rgba = tex2D(_MainTex, v.texcoord + half2(dx, -dy));
               half c20 = luminance(c20rgba);

               half4 c21rgba = tex2D(_MainTex, v.texcoord + half2(dx, 0.0));
               half c21 = luminance(c21rgba);

               half4 c22rgba = tex2D(_MainTex, v.texcoord + half2(dx, dy));
               ...
               half syAlpha = c00rgba.a * -1.0 + c01rgba.a * -2.0 + c02rgba.a * -1.0 + c20rgba.a + c21rgba.a * 2.0 +
                ...
                half4 color = half4(_OutLineColor.rgb, _OutLineColor.a * outline * v.color.a);
                return color;
           }
           ...
           half4 frag(v2f v) : SV_Target
           {
               ...
               half2 m = saturate((_ClipRect.zw - _ClipRect.xy - abs(v.mask.xy)) * v.mask.zw);
               color.a *= m.x * m.y;
               #endif
